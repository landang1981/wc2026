...
const GROUP_STAGE_MATCHES = [
  ...
  { home: 'SUI', away: 'NGA', datetime: '2026-06-19T01:00:00Z', venue: 'MetLife Stadium' },
  { home: 'ARG', away: 'CHI', datetime: '2026-06-20T22:00:00Z', venue: 'SoFi Stadium' },
  { home: 'CAN', away: 'PER', datetime: '2026-06-21T01:00:00Z', venue: 'NRG Stadium' },
  { home: 'BRA', away: 'ESP', datetime: '2026-06-21T22:00:00Z', venue: 'Rose Bowl' },
  { home: 'HON', away: 'ENG', datetime: '2026-06-22T01:00:00Z', venue: 'Mercedes-Benz Stadium' },
  { home: 'AUS', away: 'GER', datetime: '2026-06-22T22:00:00Z', venue: 'Arrowhead Stadium' },
  { home: 'JPN', away: 'FRA', datetime: '2026-06-23T01:00:00Z', venue: 'Allegiant Stadium' },
  { home: 'NZL', away: 'ITA', datetime: '2026-06-23T22:00:00Z', venue: 'AT&T Stadium' },
  { home: 'NED', away: 'POR', datetime: '2026-06-24T01:00:00Z', venue: 'Hard Rock Stadium' },
  { home: 'JAM', away: 'USA', datetime: '2026-06-24T22:00:00Z', venue: 'Lumen Field' },
  { home: 'COL', away: 'BEL', datetime: '2026-06-25T01:00:00Z', venue: "Levi's Stadium" },
  { home: 'ECU', away: 'MEX', datetime: '2026-06-25T22:00:00Z', venue: 'State Farm Stadium' },
  { home: 'URU', away: 'CRO', datetime: '2026-06-26T01:00:00Z', venue: 'FedExField' },
  { home: 'IRN', away: 'DEN', datetime: '2026-06-26T22:00:00Z', venue: 'Lincoln Financial Field' },
  { home: 'SWE', away: 'SEN', datetime: '2026-06-27T01:00:00Z', venue: 'Gillette Stadium' },
  { home: 'CMR', away: 'POL', datetime: '2026-06-27T22:00:00Z', venue: 'MetLife Stadium' },
  { home: 'NGA', away: 'SUI', datetime: '2026-06-28T01:00:00Z', venue: 'SoFi Stadium' },
  { home: 'ARG', away: 'PER', datetime: '2026-06-29T22:00:00Z', venue: 'NRG Stadium' },
  { home: 'CAN', away: 'CHI', datetime: '2026-06-30T01:00:00Z', venue: 'Rose Bowl' },
  { home: 'ENG', away: 'ESP', datetime: '2026-06-30T22:00:00Z', venue: 'Mercedes-Benz Stadium' },
  { home: 'BRA', away: 'HON', datetime: '2026-07-01T01:00:00Z', venue: 'Arrowhead Stadium' },
  { home: 'FRA', away: 'GER', datetime: '2026-07-01T22:00:00Z', venue: 'Allegiant Stadium' },
  { home: 'JPN', away: 'AUS', datetime: '2026-07-02T01:00:00Z', venue: 'AT&T Stadium' },
  { home: 'POR', away: 'ITA', datetime: '2026-07-02T22:00:00Z', venue: 'Hard Rock Stadium' },
  { home: 'NED', away: 'NZL', datetime: '2026-07-03T01:00:00Z', venue: 'Lumen Field' },
  { home: 'USA', away: 'BEL', datetime: '2026-07-03T22:00:00Z', venue: "Levi's Stadium" },
  { home: 'COL', away: 'JAM', datetime: '2026-07-04T01:00:00Z', venue: 'State Farm Stadium' },
  { home: 'MEX', away: 'CRO', datetime: '2026-07-04T22:00:00Z', venue: 'FedExField' },
  { home: 'ECU', away: 'URU', datetime: '2026-07-05T01:00:00Z', venue: 'Lincoln Financial Field' },
  { home: 'SEN', away: 'DEN', datetime: '2026-07-05T22:00:00Z', venue: 'Gillette Stadium' },
  { home: 'IRN', away: 'SWE', datetime: '2026-07-06T01:00:00Z', venue: 'MetLife Stadium' },
  { home: 'SUI', away: 'POL', datetime: '2026-07-06T22:00:00Z', venue: 'SoFi Stadium' },
  { home: 'NGA', away: 'CMR', datetime: '2026-07-07T01:00:00Z', venue: 'NRG Stadium' },
]

function generateSQL(): string {
  const esc = (s: string) => s.replace(/'/g, "''")
  const teamRows = WC2026_TEAMS.map(
    (t) => "  ('" + esc(t.name) + "', '" + t.country_code + "', " + (t.group_name ? "'" + t.group_name + "'" : 'NULL') + ", '" + t.flag_emoji + "')"
  ).join(',\n')

  const matchRows = GROUP_STAGE_MATCHES.map(
    (m) => "  ((SELECT id FROM teams WHERE country_code = '" + m.home + "'), (SELECT id FROM teams WHERE country_code = '" + m.away + "'), 'group_stage', '" + m.datetime + "', '" + m.venue.replace(/'/g, "''") + "', 'SCHEDULED')"
  ).join(',\n')

  return "-- === WC2026 Seed Data (auto-generated) ===\n" +
...
async function main() {
  console.log('🌱 WC2026 Seed Script\n')

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.warn('⚠️  Missing Supabase env vars. Generating SQL file instead.\n')
    const outDir = path.join(__dirname, '..', 'supabase', 'seeds')
    if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true })
    fs.writeFileSync(path.join(outDir, 'seed_data.sql'), generateSQL(), 'utf-8')
    console.log('✅ Generated supabase/seeds/seed_data.sql')
  ...
  for (const team of WC2026_TEAMS) {
    const { error } = await supabase.from('teams').upsert(team, { onConflict: 'country_code' })
    if (error) { console.error('  ❌', team.name, error.message) }
    else { teamCount++ }
  }
  console.log('  ✅', teamCount, 'teams')

  console.log('\n⚽ Seeding group stage matches...')
  let matchCount = 0
  for (const m of GROUP_STAGE_MATCHES) {
    const { data: h } = await supabase.from('teams').select('id').eq('country_code', m.home).single()
    const { data: a } = await supabase.from('teams').select('id').eq('country_code', m.away).single()
    if (!h || !a) { console.error('  ❌ Team not found:', m.home, m.away); continue }
    const { error } = await supabase.from('matches').insert({
      home_team_id: h.id, away_team_id: a.id, round_id: 'group_stage',
      match_datetime: m.datetime, venue: m.venue, status: 'SCHEDULED',
    })
    if (error) { console.error('  ❌', m.home, 'vs', m.away, error.message) }
    else { matchCount++ }
  }
