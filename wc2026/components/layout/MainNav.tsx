  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-3 left-3 z-50 lg:hidden w-9 h-9 flex items-center justify-center rounded-chip bg-pitch-800 border border-pitch-600 text-white hover:bg-pitch-700 transition-colors"
      ...
      {/* Sidebar */}
      <aside className={cn(
        'w-64 border-r border-pitch-700 bg-pitch-900 relative flex-shrink-0',
        // Mobile: overlay on top
        'fixed inset-y-0 left-0 z-40 transform transition-transform duration-200 lg:relative lg:transform-none',
        isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      )}>
        <div className="p-4">
          <div className="mb-6">
            <Link href="/matches" className="flex items-center gap-2" onClick={() => setIsOpen(false)}>
